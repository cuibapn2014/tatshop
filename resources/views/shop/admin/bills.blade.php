...
<hr/>
<div class="col-12 w-100 p-0 text-center d-inline-flex">
<p class="col-3 mr-2 bg-warning p-3 text-white float-left font-weight-light" style="font-size:36px;"><i class="bi bi-clipboard"></i> {{count($confirm)}}</p>
<p class="col-3 mr-2 bg-success p-3 text-white float-left font-weight-light" style="font-size:36px;"><i class="bi bi-truck"></i> {{count($transfering)}}</p>
<p class="col-3 mr-2 bg-info p-3 text-white float-left font-weight-light" style="font-size:36px;"><i class="bi bi-check-circle"></i> {{count($recieved)}}</p>
</div>
@if(session('notice'))
...
<div class="col-lg-3 col-md-6 col-sm-12 border p-2 d-inline-block m-1" style="height:auto">
    <div class="col-12">
        ...
        <p class="mb-1"><i class="fas fa-ticket-alt text-danger"></i> {{$bill->discount}}% ~
            {{number_format((($bill->discount/100 + 1) * $bill->total) * ($bill->discount/100))}}đ</p>
        ...
        <p class="mb-1">Thanh toán: 
            @php
            switch($bill->pay){
            case 1:
                echo 'COD'; break;
            case 2:
                echo 'VNPay - Đã thanh toán';break;
            }
        @endphp
        </p>
...
<div class="modal fade" id="exampleModal_{{$bill->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      Hành động này sẽ xóa hóa đơn khỏi hệ thống và không thể khôi phục lại. Bạn có chắc chắn muốn xóa hóa đơn của {{$bill->customer}}? 
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Hủy</button>
        <a href="admin/deleteBill/{{$bill->id}}"><button type="button" class="btn btn-danger">Xóa ngay</button></a>
      </div>
    </div>
  </div>
</div>
