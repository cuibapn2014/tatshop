...
<h2 class="display-4" style="font-size:36px;">Quản lý khách hàng</h2>
  ...
  @foreach($customer as $customer)
    <tr>
	  ...
	  <td>
		<a class="btn btn-outline-info rounded-0" href="admin/dashboard/edit-user/{{$customer->id}}">Edit <i class="far fa-edit"></i></a>
		<button class="btn btn-danger rounded-0" data-toggle="modal" data-target="#exampleModal_{{$customer->id}}">Xóa <i class="far fa-trash-alt"></i></button>
    </tr>
	<div class="modal fade" id="exampleModal_{{$customer->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      Hành động này sẽ xóa khách hàng khỏi hệ thống và không thể khôi phục lại. Bạn có chắc chắn muốn xóa <strong>{{$customer->name}}</strong>? 
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Hủy</button>
        <a href="admin/dashboard/delete-user/{{$customer->id}}"><button type="button" class="btn btn-danger">Xóa ngay</button></a>
      </div>
    </div>
  </div>
</div>
