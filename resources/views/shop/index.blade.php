...
<div class="col-lg-9 col-md-12 col-sm-12 float-left m-0">
    ...
    <div class="container-fluid padding p-0">
        ...
        if(count($img) > 1){
		foreach($img as $img){
		echo '<li class="float-left rounded-circle mr-2 more-img" data-id="'.$product->id.'" data-src="'.$img.'" style="background-image:url('.$img.');"></li>';
		$i++;
		}
    }
	}
            ...
            <p class="p-1 act">
                <a class="btn btn-warning btn-md" href="san-pham/{{$product->id}}/{{$product->_link}}">Mua ngay <i class="bi bi-bag-fill"></i></a>
        </div>
        </a>
        @endforeach
        <div class="container-fluid padding text-center" style="clear:both;">
            <a class="btn btn-outline-info btn-lg col-6 my-2 text-center font-weight-light" href="san-pham">Xem tất
                cả</a>
        </div>
        @else
