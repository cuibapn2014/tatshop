...
<div class="col-lg-9 col-md-12 col-sm-12 float-left m-0 d-flex flex-column">
    <div class="container-fluid padding p-0">
        @foreach($cate as $cate)
        <h2 class="display-4 mt-3 fs-3">{{$cate->category}}</h2>
        <div id="box_{{$cate->id}}" class="col-12 float-left p-0">
            ...
            @endphp
            @for($i;$i >= $end;$i--)
            <a href="san-pham/{{$cate->product[$i]->id}}/{{$cate->product[$i]->str_slug}}" title="{{$cate->product[$i]->title}}">
                <div class="card col-lg-3 col-md-4 col-sm-12 p-0 m-1 float-left border-0">
                    ...
                    <div class="thumbnail">
                        ...
                        @if($cate->product[$i]->qty <= 0) <p class="p-4">Hết hàng</p>
                            @endif
                    </div>
                    <div class="w-100 col-12 d-inline-block mt-3" style="overflow:hidden;max-height:30px;">
                        <ul>
                            ...
                            if(count($data) > 1){
                            foreach($data as $img){
                            echo '<li class="float-left rounded-circle mr-2 more-img" data-id="'.$cate->product[$i]->id.'" data-src="'.$img->image.'" style="background-image:url('.$img->image.');"></li>';
                            }
                            }
                            }
                            @endphp
                    ...
                    <p class="card-title p-1 text-dark mb-0 font-weight-bold text-left">{{$cate->product[$i]->title}}</p>
                    <p class="p-1 mb-0 font-weight-bold text-info fs-5">
                        @if($cate->product[$i]->discount > 0)
                        <s class="font-weight-light fs-6 text-dark">{{number_format($cate->product[$i]->price)}}<u>đ</u></s>
